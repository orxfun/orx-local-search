...
fn reduce<N: Neighborhood>(
    mv1: &EvalMove<N>,
    mv2: &EvalMove<N>,
) -> <N::Problem as Problem>::ObjectiveUnit {
    <<N::Problem as Problem>::Objective as Objective>::reduce(mv1.value, mv2.value)
}
    ...
    pub fn new(next1: Option<EvalMove<N>>, next2: Option<EvalMove<N>>) -> Self {
        match (next1, next2) {
            (Some(mv1), Some(mv2)) => match mv1.mv.cmp(&mv2.mv) {
                Ordering::Equal => {
                    let objective_value = reduce(&mv1, &mv2);
